...
#include "mobi_msgs/Velocities.h"
...
void PIDCallback(const mobi_msgs::PID& pid);

ros::NodeHandle nh;

ros::Subscriber<geometry_msgs::Twist> cmd_sub("cmd_vel", commandCallback);
ros::Subscriber<mobi_msgs::PID> pid_sub("pid", PIDCallback);

mobi_msgs::Imu raw_imu_msg;
ros::Publisher raw_imu_pub("raw_imu", &raw_imu_msg);

mobi_msgs::Velocities raw_vel_msg;
...
void PIDCallback(const mobi_msgs::PID& pid)
{
    //callback function every time PID constants are received from mobi_pid for tuning
...
void moveBase()
{
    ...
    req_rpm = kinematics.calculateRPM(Kinematics::MOBI_BASE, g_req_linear_vel_x, g_req_linear_vel_y, g_req_angular_vel_z);
...
void publishVelocities()
{
    ...
    vel = kinematics.getVelocities(Kinematics::MOBI_BASE, motor1.getRPM(), motor2.getRPM(), motor3.getRPM(), motor4.getRPM());
